...
class Gator(nn.Module):
    def __init__(self, config: GatorConfig) -> None:
        ...
        if self._config.predict_position:
            ...
            self._decoder_pred = nn.Linear(
                self._config.dec_emb_dim, self._patch_embed.num_patches
            )
    ...
    def _forward_decoder(
        ...
        for block in self._decoder_blocks:
            x, context = block(
                x, context, xpos=None, ypos=None, use_sa_rope=False, use_ca_rope=False
            )
        x = self._decoder_norm(x)
        return x

    def forward(
        self, img1: torch.Tensor, img2: torch.Tensor, shuffle_ratio: float | None = None
    ):
        """
        ...
        **important** When calculating the loss do not forget to remove the register tokens
        ...
        img1_enc, img1_pos, img1_gt_pos = self._forward_encoder(
            img1, shuffle=True, shuffle_ratio=shuffle_ratio
        )
