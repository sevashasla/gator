...
class Args:
    ...
    # DeiT-S: deit_small_patch16_224 (embed_dim=384)
    # DeiT-B: deit_base_patch16_224  (embed_dim=768)
...
class JigsawClassifier(LightningModule):
    def __init__(self, model: nn.Module, embed_dim: int, lr: float, total_steps: int, warmup_steps: int):
        super().__init__()
...
class ImageNetDataset(Dataset):
    """Reads label directly from the folder name (0-999), same as MAE/Gator/CroCo scripts."""
...
def save_metrics(module: JigsawClassifier, path: str = "metrics_jigsaw.json"):
    with open(path, "w") as f:
        json.dump(module.history, f, indent=2)
    print(f"Metrics saved to {path}")


def plot(module: JigsawClassifier, path: str = "training_curves_jigsaw.png"):
    h = module.history
...
def main(args: Args):
    ...
    # Build backbone via timm — no classification head
    model = timm.create_model(args.model_name, pretrained=False, num_classes=0)
    embed_dim = model.embed_dim

    # Load Jigsaw checkpoint — keys are under 'net'
    ckpt = torch.load(args.checkpoint_path, map_location="cpu")
    state_dict = ckpt["net"]

    # Keep only backbone keys
    backbone_keys = set(model.state_dict().keys())
    state_dict = {k: v for k, v in state_dict.items() if k in backbone_keys}
    ...
    print(f"embed_dim={embed_dim}, steps/epoch={steps_per_epoch}, total_steps={total_steps}, warmup_steps={warmup_steps}", flush=True)

    module = JigsawClassifier(
        model=model,
        embed_dim=embed_dim,
        lr=args.lr,
    ...
    wandb_logger = WandbLogger(project="jigsaw-finetune")

    checkpoint_callback = ModelCheckpoint(
        dirpath="/scratch/izar/mayila/jigsaw_checkpoints_50ep",
        filename="best",
        monitor="val_top1",
        mode="max",
        save_top_k=1,
